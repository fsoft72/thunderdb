# PageFile + FSM Implementation Plan (Sub-project 2)

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implement multi-page file management with a Free Space Map for efficient page allocation, building on the Page struct from sub-project 1.

**Architecture:** `PageFile` manages a file as a sequence of 8KB pages. Page 0 is the FSM (1 byte per page = free space / 32). Data pages start at page_id 1. Reads use mmap for zero-copy access; writes use direct File I/O with lazy mmap remap. The FSM enables O(n) page lookup for inserts where n = number of pages.

**Tech Stack:** Rust, memmap2 (already a dependency), existing Page struct from `src/storage/page.rs`.

---

### Task 1: Create page_file.rs with PageFile struct and open/create

**Files:**
- Create: `src/storage/page_file.rs`
- Modify: `src/storage/mod.rs`

- [ ] **Step 1: Register the module**

In `src/storage/mod.rs`, add after `pub mod page;`:
...
- [ ] **Step 2: Create page_file.rs with struct and open()**
...
Run: `cargo test --lib page_file::tests 2>&1 | tail -10`
Expected: 2 tests pass.

- [ ] **Step 4: Commit**

```bash
git add src/storage/mod.rs src/storage/page_file.rs
git commit -m "Add PageFile with open/create and FSM initialization"
```

---

### Task 2: Add read_page() and write_page()

**Files:**
- Modify: `src/storage/page_file.rs`

- [ ] **Step 1: Write test for read/write round-trip**
...
- [ ] **Step 2: Implement read_page() and write_page()**

Add to the `PageFile` impl block:
...
```bash
git add src/storage/page_file.rs
git commit -m "Add read_page, write_page, allocate_page to PageFile"
```

---

### Task 3: Add FSM operations (update_fsm, find_page_with_space)

**Files:**
- Modify: `src/storage/page_file.rs`

- [ ] **Step 1: Write tests for FSM**
...
```bash
git add src/storage/page_file.rs
git commit -m "Add FSM operations: update_fsm, find_page_with_space"
```

---

### Task 4: Add integration test and persistence round-trip
...
```bash
git add src/storage/page_file.rs
git commit -m "Add PageFile integration tests: multi-page insert and persistence"
```

---

### Task 5: Update CHANGES.md and run full test suite
...
Expected: all tests pass (page_file tests + all existing tests).
...
```bash
git add CHANGES.md
git commit -m "Complete PageFile sub-project 2: CHANGES.md"
```
