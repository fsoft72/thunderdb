# Slotted Page Implementation Plan (Sub-project 1)

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implement an 8KB slotted page data structure with Ctid addressing, slot directory, row CRUD, compaction, and single-column extraction — the foundation for replacing ThunderDB's append-only storage format.

**Architecture:** A `Page` struct wraps an `[u8; 8192]` array with a parsed header. Slot directory grows forward from byte 24; row data grows backward from byte 8191. A `Ctid(page_id, slot_index)` addresses rows and packs into u64 for B-tree index compatibility. Rows inside pages use a compact format (u16 col_count, no row_id). Everything lives in a single new file `src/storage/page.rs` with no external dependencies beyond `crate::error` and `crate::storage::value::Value`.

**Tech Stack:** Rust, no new dependencies.

---

### Task 1: Create page.rs with constants, Ctid, and PageType
...
In `src/storage/mod.rs`, add after line 15 (`pub mod small_string;`):

```rust
pub mod page;
```

- [ ] **Step 2: Create page.rs with constants and Ctid**
...
Run: `cargo test --lib page::tests 2>&1 | tail -10`
Expected: 4 tests pass.

- [ ] **Step 4: Commit**

```bash
git add src/storage/mod.rs src/storage/page.rs
git commit -m "Add page module with Ctid and PageType"
```

---

### Task 2: Add Page struct with new(), from_bytes(), to_bytes()

**Files:**
- Modify: `src/storage/page.rs`

- [ ] **Step 1: Write test for page creation and round-trip**

Add to the `tests` module in `src/storage/page.rs`:
...
- [ ] **Step 2: Implement Page struct**

Add before the `#[cfg(test)]` block in `src/storage/page.rs`:
...
impl PageHeader {
    /// Parse header from the first 24 bytes of page data.
    ...
    /// Write header back into page data.
...
impl Page {
    ...
    pub fn can_fit(&self, row_size: usize) -> bool {
        if self.header.first_free_slot != INVALID_SLOT {
            // Reusing a free slot: only need space for row data
            self.free_space() >= row_size
        } else {
            // Need a new slot entry + row data
...
Run: `cargo test --lib page::tests 2>&1 | tail -10`
Expected: 6 tests pass (4 old + 2 new).

- [ ] **Step 4: Commit**

```bash
git add src/storage/page.rs
git commit -m "Add Page struct with header, new(), from_bytes(), to_bytes()"
```

---

### Task 3: Add serialize_row_for_page() and insert_row() / get_row()

**Files:**
- Modify: `src/storage/page.rs`

- [ ] **Step 1: Write tests for row serialization and insert/get**
...
```rust
    ...
    fn test_serialize_row_for_page() {
        ...
        // col_count(2) + 2 offsets(4) + Int32(5) + Varchar(1+4+5=10) = 21 bytes
    ...
    fn test_insert_multiple_rows() {
        ...
        let first = page.get_row(0).unwrap();
        let col_count = u16::from_le_bytes(first[0..2].try_into().unwrap());
        assert_eq!(col_count, 2);

        let last = page.get_row(49).unwrap();
    ...
    fn test_insert_full_page_error() {
        let mut page = Page::new(0);
        // Insert rows until page is full
...
- [ ] **Step 2: Implement `serialize_row_for_page()`**

Add before the `Page` struct in `src/storage/page.rs`:
...
pub fn serialize_row_for_page(values: &[Value]) -> Vec<u8> {
    ...
    // Serialize values to compute offsets
    ...
    // Build the row: col_count + offsets + values
...
- [ ] **Step 3: Implement `insert_row()` and `get_row()`**

Add these methods to the `Page` impl block:

```rust
    /// Insert a row into the page. Returns the slot index.
    ///
    /// The row data is written at the end of the page (growing backward).
    /// A slot entry is allocated from the free list or created at the end
    ...
    pub fn insert_row(&mut self, row_data: &[u8]) -> Result<u16> {
        ...
        let slot_index = if self.header.first_free_slot != INVALID_SLOT {
            ...
            // Read next-free pointer from the length field of this free slot
    ...
    /// Get the raw row data for a slot. Returns None if the slot is free
    /// or out of bounds.
...
Run: `cargo test --lib page::tests 2>&1 | tail -10`
Expected: 11 tests pass (6 old + 5 new).

- [ ] **Step 5: Commit**

```bash
git add src/storage/page.rs
git commit -m "Add serialize_row_for_page, insert_row, get_row"
```

---

### Task 4: Add delete_row() and free slot reuse

**Files:**
- Modify: `src/storage/page.rs`

- [ ] **Step 1: Write tests for delete and slot reuse**
...
```rust
    ...
    fn test_slot_reuse_after_delete() {
        ...
        assert_eq!(page.active_count(), 1);
        assert_eq!(page.slot_count(), 3); // slots not reclaimed

        // Next insert reuses slot 1 (head of free list, LIFO)
        let data4 = serialize_row_for_page(&vec![Value::Int32(4)]);
        let s_new = page.insert_row(&data4).unwrap();
        assert_eq!(s_new, s1); // reused slot 1

        // Next insert reuses slot 0
        let data5 = serialize_row_for_page(&vec![Value::Int32(5)]);
        let s_new2 = page.insert_row(&data5).unwrap();
        assert_eq!(s_new2, s0); // reused slot 0
    }

    #[test]
    fn test_delete_invalid_slot() {
        let mut page = Page::new(0);
        assert!(!page.delete_row(0));   // no slots exist
        assert!(!page.delete_row(999)); // way out of bounds
    }
```

- [ ] **Step 2: Implement `delete_row()`**

Add to the `Page` impl block:

```rust
    /// Delete a row by slot index. Returns false if the slot is already
    /// free or out of bounds.
    ///
    /// The slot is added to the free list for reuse. Row data space is
    /// NOT reclaimed — use `compact()` to defragment.
    pub fn delete_row(&mut self, slot_index: u16) -> bool {
        ...
        // Mark slot as free: set offset to INVALID_SLOT,
        // store pointer to previous free list head in the length field
...
Run: `cargo test --lib page::tests 2>&1 | tail -10`
Expected: 14 tests pass (11 old + 3 new).

- [ ] **Step 4: Commit**

```bash
git add src/storage/page.rs
git commit -m "Add delete_row with free slot list"
```

---

### Task 5: Add compact() for defragmentation

**Files:**
- Modify: `src/storage/page.rs`

- [ ] **Step 1: Write test for compaction**
...
```rust
    ...
    fn test_compact() {
        ...
        // Remaining rows still readable and correct
        let r0 = page.get_row(slots[0]).unwrap();
        let col_count = u16::from_le_bytes(r0[0..2].try_into().unwrap());
        assert_eq!(col_count, 2);

        let r2 = page.get_row(slots[2]).unwrap();
        assert!(!r2.is_empty());

        let r4 = page.get_row(slots[4]).unwrap();
        assert!(!r4.is_empty());
...
```bash
git add src/storage/page.rs
git commit -m "Add page compaction for defragmentation"
```

---

### Task 6: Add value_at() for single-column extraction

**Files:**
- Modify: `src/storage/page.rs`

- [ ] **Step 1: Write test for value_at**
...
```rust
    ...
    pub fn value_at(&self, slot_index: u16, col_idx: usize) -> Result<Value> {
        ...
        let offsets_start = 2usize; // after u16 col_count
...
```bash
git add src/storage/page.rs
git commit -m "Add Page::value_at() for single-column extraction"
```

---

### Task 7: Add page serialization round-trip with data and update CHANGES.md
    ...
    fn test_full_roundtrip_with_data() {
        ...
        for i in 0..10u16 {
            if i == 3 || i == 7 {
                ...
                let row = restored.get_row(i).unwrap();
...
Expected: all tests pass (page module + all existing tests).
...
- **8KB slotted page format**: Page header (24 bytes) + slot directory (grows →) + row data (grows ←), supporting insert, get, delete, and compaction
...
```bash
git add src/storage/page.rs CHANGES.md
git commit -m "Complete slotted page sub-project 1: comprehensive tests and CHANGES.md"
```
